...
X_imp = SimpleImputer(strategy="median").fit_transform(X_brut.values)
...
with t6:
    ...
    st.markdown("Évaluation quantitative de chaque composant du pipeline ML.")
    ...
    # Scree plot
    fig_scree = go.Figure()
    fig_scree.add_bar(x=list(range(1, len(pca_exp)+1)),
                      y=pca_exp * 100,
                      name="Var. individuelle",
                      marker_color="#2E5496")
    fig_scree.add_scatter(x=list(range(1, len(pca_cum)+1)),
                          y=pca_cum * 100,
                          mode="lines+markers",
                          name="Var. cumulée",
    ...
    col_pca1.plotly_chart(fig_scree, use_container_width=True)

    # Tableau variance par composante
    ...
    col_pca2.markdown("**Variance par composante (top 15)**")
    col_pca2.dataframe(df_pca_tab, use_container_width=True, height=340)
    ...
    fig_dens = go.Figure(go.Histogram2dContour(
        x=X_u2[:, 0], y=X_u2[:, 1],
        colorscale="Blues", ncontours=18,
        showscale=True))
    fig_dens.update_layout(
        title="Densité de la projection UMAP 2D",
        xaxis_title="UMAP1", yaxis_title="UMAP2",
        template="plotly_white", height=380,
        font=dict(family="Arial", size=11))
    col_u1.plotly_chart(fig_dens, use_container_width=True)

    # Distribution des coordonnées UMAP
    fig_umap_dist = make_subplots(rows=1, cols=2,
        subplot_titles=["Distribution UMAP1", "Distribution UMAP2"])
    fig_umap_dist.add_trace(
        go.Histogram(x=X_u2[:, 0], nbinsx=30,
                     marker_color="#7B2D8B", name="UMAP1"), row=1, col=1)
    fig_umap_dist.add_trace(
        go.Histogram(x=X_u2[:, 1], nbinsx=30,
                     marker_color="#2E5496", name="UMAP2"), row=1, col=2)
    fig_umap_dist.update_layout(
        template="plotly_white", height=380, showlegend=False,
        font=dict(family="Arial", size=11))
    col_u2.plotly_chart(fig_umap_dist, use_container_width=True)
    ...
    h3.metric("Silhouette Score",      f"{sil:.3f}",
              delta="✅ Bon (> 0.5)" if sil > 0.5 else ("⚠️ Moyen" if sil > 0.25 else "❌ Faible"))
    h4.metric("Davies-Bouldin Index",  f"{dbi:.3f}",
              delta="✅ Bon (< 1.5)" if dbi < 1.5 else ("⚠️ Moyen" if dbi < 2.5 else "❌ Élevé"))
    ...
    cl_sizes["Label"] = cl_sizes["Cluster"].apply(
        lambda x: "Bruit (-1)" if x == -1 else f"C{x}")
    cl_sizes["Couleur"] = cl_sizes["Cluster"].apply(
        lambda x: "#C00000" if x == -1 else "#2E5496")
    ...
    fig_cls.update_layout(
        title="Taille de chaque cluster HDBSCAN",
        xaxis_title="Cluster", yaxis_title="Nombre de fournisseurs",
        template="plotly_white", height=360,
        font=dict(family="Arial", size=11))
    col_h1.plotly_chart(fig_cls, use_container_width=True)

    # Score moyen par cluster
    cl_score = pd.DataFrame({"Cluster": labels, "Score": score_100})
    cl_score["Label"] = cl_score["Cluster"].apply(
        lambda x: "Bruit (-1)" if x == -1 else f"C{x}")
    cl_agg = cl_score.groupby("Label")["Score"].agg(
        ["mean", "std", "min", "max"]).round(2).reset_index()
    cl_agg.columns = ["Cluster", "Moy.", "Écart-type", "Min", "Max"]
    col_h2.markdown("**Score composite par cluster**")
    col_h2.dataframe(cl_agg, use_container_width=True, height=340)

    # Jauge Silhouette
    fig_sil_gauge = go.Figure(go.Indicator(
        mode="gauge+number+delta",
        value=sil,
        delta={"reference": 0.5, "increasing": {"color": "#375623"},
               "decreasing": {"color": "#C00000"}},
        gauge={
            "axis": {"range": [-1, 1], "tickwidth": 1},
            "bar":  {"color": "#2E5496"},
            "steps": [
                {"range": [-1, 0],   "color": "#ffcccc"},
                {"range": [0, 0.25], "color": "#ffe0b2"},
                {"range": [0.25, 0.5], "color": "#fff9c4"},
                {"range": [0.5, 1],  "color": "#ccffcc"},
            ],
            "threshold": {"line": {"color": "#C00000", "width": 3},
                          "thickness": 0.8, "value": 0.5}},
        title={"text": "Silhouette Score<br><span style='font-size:0.8em'>[-1 → 1 | Bon > 0.5]</span>"},
        number={"font": {"size": 40}}))
    fig_sil_gauge.update_layout(height=300, font=dict(family="Arial", size=12))

    fig_dbi_gauge = go.Figure(go.Indicator(
        mode="gauge+number+delta",
        value=dbi,
        delta={"reference": 1.5, "increasing": {"color": "#C00000"},
               "decreasing": {"color": "#375623"}},
        gauge={
            "axis": {"range": [0, 5], "tickwidth": 1},
            "bar":  {"color": "#C55A11"},
            "steps": [
                {"range": [0,   1.5], "color": "#ccffcc"},
                {"range": [1.5, 2.5], "color": "#fff9c4"},
                {"range": [2.5, 5],   "color": "#ffcccc"},
            ],
            "threshold": {"line": {"color": "#C00000", "width": 3},
                          "thickness": 0.8, "value": 1.5}},
        title={"text": "Davies-Bouldin Index<br><span style='font-size:0.8em'>[0 → ∞ | Bon < 1.5]</span>"},
        number={"font": {"size": 40}}))
    fig_dbi_gauge.update_layout(height=300, font=dict(family="Arial", size=12))

    g1, g2 = st.columns(2)
    g1.plotly_chart(fig_sil_gauge, use_container_width=True)
    g2.plotly_chart(fig_dbi_gauge, use_container_width=True)
    ...
    i4.metric("Score IF moyen (norm.)", f"{if_sc.mean()*100:.1f}/100")

    col_i1, col_i2 = st.columns(2)

    fig_if_hist = go.Figure()
    fig_if_hist.add_trace(go.Histogram(
        x=if_sc * 100, nbinsx=40,
        marker_color="#C00000", opacity=0.75, name="Score IF"))
    fig_if_hist.add_vline(
        x=if_sc.mean() * 100, line_dash="dash", line_color="#1F3864",
    ...
    fig_if_hist.update_layout(
        ...
        template="plotly_white", height=360,
        font=dict(family="Arial", size=11))
    col_i1.plotly_chart(fig_if_hist, use_container_width=True)

    # Score IF par cluster
    df_if_cl = pd.DataFrame({"Label": ["Bruit (-1)" if l == -1 else f"C{l}" for l in labels],
                              "Score_IF": if_sc * 100})
    fig_if_box = px.box(df_if_cl, x="Label", y="Score_IF", color="Label",
                         title="Score IF par cluster HDBSCAN",
                         template="plotly_white", height=360, points="outliers")
    fig_if_box.update_layout(showlegend=False, font=dict(family="Arial", size=11))
    col_i2.plotly_chart(fig_if_box, use_container_width=True)
    ...
    v2.metric("Époques d'entraînement", 60)
    ...
    fig_vae_hist = go.Figure()
    fig_vae_hist.add_trace(go.Histogram(
        x=vae_sc * 100, nbinsx=40,
        marker_color="#C55A11", opacity=0.75, name="Score VAE"))
    fig_vae_hist.add_vline(
        x=vae_sc.mean() * 100, line_dash="dash", line_color="#1F3864",
        annotation_text=f"Moy {vae_sc.mean()*100:.1f}")
    fig_vae_hist.update_layout(
        title="Distribution des erreurs de reconstruction VAE",
        xaxis_title="Score VAE (0–100)", yaxis_title="Fréquence",
        template="plotly_white", height=360,
        font=dict(family="Arial", size=11))
    col_v1.plotly_chart(fig_vae_hist, use_container_width=True)

    # Corrélation IF vs VAE
    fig_corr = px.scatter(
    ...
    fig_corr.add_annotation(x=80, y=10,
        text=f"r = {corr_val:.3f}", showarrow=False,
        font=dict(size=14, color="#1F3864"),
        bgcolor="white", bordercolor="#1F3864", borderwidth=1)
    col_v2.plotly_chart(fig_corr, use_container_width=True)

    # ── Score composite ──────────────────────────────────────────
    st.markdown("---")
    st.markdown("### 🏆 Score composite — Synthèse du pipeline")
    sc1, sc2, sc3, sc4, sc5 = st.columns(5)
    sc1.metric("Score moyen",   f"{score_100.mean():.1f}/100")
    sc2.metric("Score médian",  f"{np.median(score_100):.1f}/100")
    sc3.metric("Score max",     f"{score_100.max():.1f}/100")
    sc4.metric("Écart-type",    f"{score_100.std():.1f}")
    sc5.metric("% Rouge",       f"{n_rouge/len(df)*100:.1f}%")

    # Poids utilisés
    st.markdown("**Poids appliqués dans le score composite :**")
    df_poids = pd.DataFrame({
        "Composante": ["Cluster HDBSCAN", "Anomalie (IF+VAE)", "Temporel (DTW)", "VAE seul"],
        "Poids (%)": [f"{p_w_cl*100:.0f}%", f"{p_w_an*100:.0f}%",
                      f"{p_w_te*100:.0f}%", f"{p_w_dt*100:.0f}%"],
        "Source": ["HDBSCAN", "Isolation Forest + VAE", "PSI / PELT / OTD", "VAE reconstruction"],
    })
    pw1, pw2 = st.columns([1, 2])
    pw1.dataframe(df_poids, use_container_width=True, hide_index=True)

    fig_poids = go.Figure(go.Pie(
    ...
    fig_poids.update_layout(
        title="Répartition des poids — Score composite",
        height=300, template="plotly_white",
        font=dict(family="Arial", size=11),
        showlegend=False, margin=dict(t=50, b=10))
    pw2.plotly_chart(fig_poids, use_container_width=True)

    # Tableau récapitulatif
    st.markdown("---")
    st.markdown("### 📋 Tableau récapitulatif — Qualité du pipeline")
    recap_data = {
        "Algorithme": ["PCA", "UMAP", "HDBSCAN", "Isolation Forest", "VAE"],
        "Rôle": [
            "Réduction dimensionnalité",
            "Projection non-linéaire 2D/3D",
            "Clustering densité",
            "Détection anomalies (arbre)",
...
st.markdown("---")
st.caption("Université Mohammed V — Rabat · Master M.I.E.L · 2024–2025")
